...
class GameManager:
    ...
    def save(self, path: str) -> None:
     ...
     # [Fix] 補回這個遺失的方法
    def accept_new_quest(self, name: str, description: str, target_count: int, reward: int) -> None:
        new_quest = {
            "name": name,
        ...
        self.save("saves/game0.json")
        Logger.info(f"Accepted new quest: {name}")        
    @classmethod
    ...
    def from_dict(cls, data: dict[str, object]) -> "GameManager":
        ...
        # [New] 讀取任務 (如果存檔沒有任務資料，就給一個預設的)
        gm.quest = data.get("quest", {
            "name": "Daily Quest",
